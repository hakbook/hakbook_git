...
public class ReviewDAO {
	...
	public List<ReviewDTO> rvList(int start, int end) {
		List<ReviewDTO> relist=new ArrayList<>();
		...
		try {
			...
			sql.append("from ( select num,writer,subject,review_date,readcount,filename,filesize,ip ");
			sql.append("from review order by num desc) A) ");
	...
	public void write(ReviewDTO dto) {
		...
		try {
			...
			sql.append("( (select nvl(max(num)+1,1) from review),?,?,?,?,?,?,?)");
			pstmt = conn.prepareStatement(sql.toString());
	...
	public String getFileName(int num) {
		...
		try {
			conn = DB.getConn();
			String sql="select filename from review where num=?";
			pstmt=conn.prepareStatement(sql);
			rs=pstmt.executeQuery();
			if(rs.next()) {
				pstmt.setInt(1, num);
